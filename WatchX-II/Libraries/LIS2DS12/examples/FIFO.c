...
int example_main(void)
{
  ...
  if(response==MEMS_ERROR) while(1); //manage here comunication error  
  /* Set FIFO Mode */
  response = LIS2DS12_ACC_W_FMODE(0, LIS2DS12_ACC_FMODE_FIFO);   
  if(response==MEMS_ERROR) while(1); //manage here comunication error
  ...
  while(1)
  {
    ...
    if ( HAL_GPIO_ReadPin( INT1_GPIO_Port, INT1_Pin ) ) 
    {
        ...
        /* Enable again FIFO mode (sample acquisition aoutomatically stop in FIFO mode)*/
        /* Turn the FIFO off (Bypass Mode) */
        LIS2DS12_ACC_W_FMODE(0, LIS2DS12_ACC_FMODE_BYPASS);
        /* Turn the FIFO on (FIFO Mode) */
        LIS2DS12_ACC_W_FMODE(0, LIS2DS12_ACC_FMODE_FIFO);
    }
  }
}
