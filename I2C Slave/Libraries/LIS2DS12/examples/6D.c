...
static u8_t WhoAmI, rst, Status_6D;
POS_t position;
...
int example_main(void)
{
  ...
  /* 6D Configuration */
  /* Set 6D Threshold */
  response = LIS2DS12_ACC_W_6D_THS(0, LIS2DS12_ACC_6D_THS_60_degrees);
  if(response==MEMS_ERROR) while(1); //manage here comunication error  

  /* Disable 4D Reduction set */
  response = LIS2DS12_ACC_W_4D_EN(0, LIS2DS12_ACC_4D_EN_OFF);
  if(response==MEMS_ERROR) while(1); //manage here comunication error
  
  /* Enable 6D Interrupt pin */
  response = LIS2DS12_ACC_W_INT1_6D(0, LIS2DS12_ACC_INT1_6D_ON);
  if(response==MEMS_ERROR) while(1); //manage here comunication error  
  ...
  while(1)
  {
    ...
    {
        LIS2DS12_ACC_ReadReg(0, LIS2DS12_ACC_6D_SRC, &Status_6D, 1);
        
        if ( Status_6D & LIS2DS12_ACC_XL_EV_ON )  
        {
            position = X_NEG;
        }
        else if ( Status_6D & LIS2DS12_ACC_XH_EV_ON )
        {
            position = X_POS;
        }
        if ( Status_6D & LIS2DS12_ACC_YL_EV_ON )  
        {
            position = Y_NEG;
        }
        else if ( Status_6D & LIS2DS12_ACC_YH_EV_ON )
        {
            position = Y_POS;
        }
        if ( Status_6D & LIS2DS12_ACC_ZL_EV_ON )  
        {
            position = Z_NEG;        
        }
        else if ( Status_6D & LIS2DS12_ACC_ZH_EV_ON )
        {
            position = Z_POS;
        }        
    }
