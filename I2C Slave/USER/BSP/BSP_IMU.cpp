#include "BSP.h"
#include "LIS2DS12/LIS2DS12_ACC.h"
...
ComSlave_MakeNode(IMU, DataPack);
...
static uint8_t WhoAmI, rst, stepDetect, dummy;
...
void IMU_Update()
{
    ...
    if (1 || digitalRead(LIS_INT2_Pin))
    {
        ...
        if ( stepDetect == LIS2DS12_ACC_STEP_DETECT_EV_ON)
        {
            ...
            DataPack.Steps = number_of_steps.i16bit;
            isSend = false;
        }
    }
    ...
    if(!isSend)
    {
        isSend = ComSlave_ReqExecuteUserCallbackNode(&IMU_Node);
    }
}
